...
#define WINBOND_ENTER_4BADDR				0xb7 
#define WINBOND_EXIT_4BADDR					0xe9 
#define WINBOND_READ_4BADDR					0x13
#define WINBOND_PAGE_PGM_4BADDR				0x12
#define WINBOND_SECTOR_E_4BADDR				0x21
#define WINBOND_BLK_E_64K_4BARR				0xDC
...
/* Private variables ---------------------------------------------------------*/
static bool is4byteModes = false;
...
uint8_t flashEntry4ByteModes() {
	...
	FlashCsLow();
	FlashTransferData(WINBOND_ENTER_4BADDR);
	FlashCsHigh();
	FlashDelayUs(200);
...
uint8_t flashExit4ByteModes() {
	...
	FlashCsLow();
	FlashTransferData(WINBOND_EXIT_4BADDR);
	FlashCsHigh();
	FlashDelayUs(200);
...
uint8_t flashRead(uint32_t addr, uint8_t* pBuff, uint32_t NbrByte) {
	...
	if (is4byteModes) {
		FlashTransferData(WINBOND_READ_4BADDR);
		FlashTransferData((addr >> 24) & 0xFF);
		FlashTransferData((addr >> 16) & 0xFF);
		FlashTransferData((addr >> 8)  & 0xFF);
		FlashTransferData((addr >> 0)  & 0xFF); // 0000 0000 0000 0000 0000 0000 0000 0000
	}
	else {
...
uint8_t flashWrite(uint32_t addr, uint8_t* pBuff, uint32_t NbrByte) {
	...
	while(NbrByte) {
		if (NextPage) {
			...
			FlashCsLow();
			if (is4byteModes) {
				FlashTransferData(WINBOND_PAGE_PGM_4BADDR);
				FlashTransferData((addr >> 24) & 0xFF);
				FlashTransferData((addr >> 16) & 0xFF);
				FlashTransferData((addr >> 8)  & 0xFF);
				FlashTransferData((addr >> 0)  & 0xFF);
...
uint8_t flashEraseSector(uint32_t addr) {
	...
	FlashCsLow();
	if (is4byteModes) {
		FlashTransferData(WINBOND_SECTOR_E_4BADDR);
		FlashTransferData((addr >> 24) & 0xFF);
		FlashTransferData((addr >> 16) & 0xFF);
		FlashTransferData((addr >> 8)  & 0xFF);
		FlashTransferData((addr >> 0)  & 0xFF);
...
uint8_t flashEraseBlock64k(uint32_t addr) {
	...
	FlashCsLow();
	if (is4byteModes) {
		FlashTransferData(WINBOND_BLK_E_64K_4BARR);
		FlashTransferData((addr >> 24) & 0xFF);
		FlashTransferData((addr >> 16) & 0xFF);
		FlashTransferData((addr >> 8)  & 0xFF);
		FlashTransferData((addr >> 0)  & 0xFF);
...
uint8_t  flashEraseFull() {
	...
	Set_LOCK(true);
	FlashCsLow();
	FlashTransferData(WINBOND_CHIP_ERASE);
	FlashCsHigh();
	FlashDelayUs(200);
